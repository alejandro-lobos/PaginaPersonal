...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full flex justify-between  items-center  px-4 sm:px-6 lg:px-8 py-3 z-50 transition-colors duration-300 ${scrolled ? 'bg-gray-800' : 'bg-transparent'} bg-opacity-70 text-white`}>
      ...
      <ul className={`absolute lg:relative w-full lg:w-auto flex flex-col lg:flex-row transition-all ease-out duration-300  ${isOpen ? 'top-12 left-0 bg-gray-800 lg:bg-transparent block ' : 'hidden lg:flex'} text-white`}>
